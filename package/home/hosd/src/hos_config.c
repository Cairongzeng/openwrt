...
int config_get_appfilter_enable(void)
{
    ...
    if (!ctx)
        return -1;
	enable = hos_uci_get_int_value(ctx, "appfilter.global.enable");
    if (enable < 0)
        enable = 0;
...
int config_get_lan_ip(char *lan_ip, int len)
{
    ...
    if (!ctx)
        return -1;
    ret = hos_uci_get_value(ctx, "network.lan.ipaddr", lan_ip, len);
    uci_free_context(ctx);
    return ret;
}
...
int config_get_lan_mask(char *lan_mask, int len)
{
    ...
    if (!ctx)
        return -1;
    ret = hos_uci_get_value(ctx, "network.lan.netmask", lan_mask, len);
    uci_free_context(ctx);
    return ret;
}
