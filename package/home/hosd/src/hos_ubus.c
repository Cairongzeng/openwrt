...
#define CLIENT_DATA_BASE_DIR_DEFAULT "/tmp/hos/client_data"
#include "hos_app_filter.h"
#include "hos_mac_filter.h"
#include "hos_network.h"
#include "hos_system.h"
#include "hos_stat.h"
#include <fcntl.h>
#include <unistd.h>
#include <libubox/list.h>

extern hos_status_t g_hos_status;
...
struct json_object *hos_api_set_nickname(struct json_object *req_obj) {
    if (!req_obj) {
    ...
    int num = hos_uci_get_list_num(uci_ctx, "user_info", "user_info");
    char mac_str[128] = {0};
    int index = -1;
    int i;
    
    for (i = 0; i < num; i++) {
        hos_uci_get_array_value(uci_ctx, "user_info.@user_info[%d].mac", i, mac_str, sizeof(mac_str));
    ...
    if (nickname && strlen(nickname) > 0) {
        if (index == -1) {
            hos_uci_add_section(uci_ctx, "user_info", "user_info");
            index = num;  
        }
        hos_uci_set_array_value(uci_ctx, "user_info.@user_info[%d].mac", index, (char *)mac);
        hos_uci_set_array_value(uci_ctx, "user_info.@user_info[%d].nickname", index, (char *)nickname);
    } else {
        if (index >= 0) {
            char uci_option[128] = {0};
            snprintf(uci_option, sizeof(uci_option), "user_info.@user_info[%d]", index);
            hos_uci_delete(uci_ctx, uci_option);
            LOG_DEBUG("delete nickname mac = %s\n", mac);
        }
    }

    hos_uci_commit(uci_ctx, "user_info");
    reload_oaf_rule();
    uci_free_context(uci_ctx);
...
struct json_object *hos_api_dev_visit_list(struct json_object *req_obj) {
    if (!req_obj) {
        return hos_gen_api_response_data(API_CODE_ERROR, NULL);
    }
    ...
    if (!node) {
        ...
        return hos_gen_api_response_data(API_CODE_SUCCESS, root_obj);
    }
...
struct json_object *hos_api_dev_visit_time(struct json_object *req_obj) {
    if (!req_obj) {
        return hos_gen_api_response_data(API_CODE_ERROR, NULL);
    }
    ...
    return hos_gen_api_response_data(API_CODE_SUCCESS, resp_obj);
}


struct json_object *hos_api_app_class_visit_time(struct json_object *req_obj) {
    if (!req_obj) {
        return hos_gen_api_response_data(API_CODE_ERROR, NULL);
    }
...
struct json_object *hos_api_dev_list(struct json_object *req_obj) {
    int i, j;
    struct json_object *root_obj = json_object_new_object();
    struct json_object *dev_array = json_object_new_array();
...
struct json_object *hos_api_class_list(struct json_object *req_obj) {
    struct json_object *class_list = json_object_new_array();
    ...
    return hos_gen_api_response_data(API_CODE_SUCCESS, response);
}


struct json_object *hos_api_get_all_users(struct json_object *req_obj) {
    if (!req_obj) {
        return hos_gen_api_response_data(API_CODE_ERROR, NULL);
    }
...
struct json_object *hos_api_get_oaf_status(struct json_object *req_obj) {
    struct json_object *data_obj = json_object_new_object();
    char result[128] = {0};
...
struct json_object *hos_api_visit_list(struct json_object *req_obj) {
    const char *mac = NULL;
...
static int handle_set_nickname(struct ubus_context *ctx, struct ubus_object *obj,
    ...
    if (strlen(json_object_get_string(nickname_obj)) > 0) {
        ...
        hos_uci_set_array_value(uci_ctx, "user_info.@user_info[%d].mac", index, (char *)json_object_get_string(mac_obj));
        hos_uci_set_array_value(uci_ctx, "user_info.@user_info[%d].nickname", index, (char *)json_object_get_string(nickname_obj));
    }
    else{
        char uci_option[128] = {0};
...
extern hos_run_time_status_t g_af_status;
...
static int handle_get_whitelist_user(struct ubus_context *ctx, struct ubus_object *obj,
    ...
    char mac_str[128] = {0};
    int num = hos_uci_get_list_num(uci_ctx, "appfilter", "whitelist");
    for (i = 0; i < num; i++) {
        hos_uci_get_array_value(uci_ctx, "appfilter.@whitelist[%d].mac", i, mac_str, sizeof(mac_str));
        struct json_object *user_obj = json_object_new_object();
                    ...
                    struct blob_attr *msg) 
{
    ...
    for (i = 0; i < len; i++) {
        ...
        hos_uci_add_section(uci_ctx, "appfilter", "whitelist");
        hos_uci_set_value(uci_ctx, "appfilter.@whitelist[-1].mac", (char *)json_object_get_string(mac_obj));
    }
    hos_uci_commit(uci_ctx, "appfilter");
    reload_oaf_rule();
...
static struct json_object *get_dashboard_system_status(void) {
    ...
    char hos_version_buf[32] = {0};
    if (read_file_buf("/etc/hos_version", hos_version_buf, sizeof(hos_version_buf)) > 0) {
        str_trim(hos_version_buf);
        json_object_object_add(system_status, "hos_version", json_object_new_string(hos_version_buf));
    } else {

        if (read_file_buf("/etc/version", hos_version_buf, sizeof(hos_version_buf)) > 0) {
            str_trim(hos_version_buf);
            json_object_object_add(system_status, "hos_version", json_object_new_string(hos_version_buf));
        } else {
            json_object_object_add(system_status, "hos_version", json_object_new_string("Unknown"));
        }
    }
    ...
    int connections = hos_stat_read_conntrack_count();
...
static struct json_object *get_dashboard_network_status(void) {
    ...
    int work_mode = 1; 
    if (uci_ctx) {
        work_mode = hos_uci_get_int_value(uci_ctx, "appfilter.global.work_mode");
    ...
    json_object_object_add(network_status, "internet", json_object_new_int(g_hos_status.internet));
...
static int get_interface_device(const char *interface_name, char *device_name, size_t device_name_len) {
    ...
    int ret = hos_uci_get_value(uci_ctx, uci_key, device_name, device_name_len);
    uci_free_context(uci_ctx);
...
void check_and_update_monitor_device(void) {    
    ...
    int ret = hos_uci_get_value(uci_ctx, "hos.dashboard.monitor_device", monitor_device, sizeof(monitor_device));
    if (ret == 0 && strlen(monitor_device) > 0) {
        ...
        hos_uci_set_value(uci_ctx, "hos.dashboard.monitor_device", g_interface_name);
...
struct json_object *hos_api_get_dashboard_common(struct json_object *req_obj) {
    struct json_object *data_obj = json_object_new_object();
...
struct json_object *hos_api_get_hourly_top_apps(struct json_object *req_obj) {
	int i;
    struct json_object *data_obj = json_object_new_object();
...
struct json_object *hos_api_get_daily_top_apps(struct json_object *req_obj) {
    struct json_object *data_obj = json_object_new_object();
    int i;
    int count;
    if (!req_obj) {
...
struct json_object *hos_api_delete_record_files(struct json_object *req_obj) {
    struct json_object *mac_obj = json_object_object_get(req_obj, "mac");
...
struct json_object *hos_api_get_global_app_type_stats(struct json_object *req_obj) {
    struct json_object *type_obj = json_object_object_get(req_obj, "type");
...
struct json_object *hos_api_get_global_traffic_stats(struct json_object *req_obj) {
    struct json_object *date_obj = json_object_object_get(req_obj, "date");
    int hour;
    u_int32_t target_date = 0;
    int is_today = 1;
...
struct json_object *hos_api_get_daily_top_users(struct json_object *req_obj) {
	int i;
	int hour;
    LOG_DEBUG("hos_api_get_daily_top_users: called\n");
    
    struct json_object *data_obj = json_object_new_object();
    if (!data_obj) {
        LOG_ERROR("hos_api_get_daily_top_users: failed to create data_obj\n");
        return hos_gen_api_response_data(API_CODE_ERROR, NULL);
    }
    
    
    u_int32_t today = get_today_start_timestamp();
    LOG_DEBUG("hos_api_get_daily_top_users: today timestamp = %u\n", today);
    
    
    user_traffic_sort_t user_traffic_list[1024];  
    int user_count = 0;
    
    LOG_DEBUG("hos_api_get_daily_top_users: start iterating client_list\n");
    client_node_t *node = NULL;
    list_for_each_entry(node, &client_list, client) {
        if (user_count >= 1024) {
            LOG_ERROR("hos_api_get_daily_top_users: user_count reached max limit 1024\n");
            break;  
        }
        ...
        if (!today_stat) {
            LOG_DEBUG("hos_api_get_daily_top_users: client %s has no today_stat, skip\n", node->mac);
            continue;
        }
        ...
        LOG_DEBUG("hos_api_get_daily_top_users: client %s, up_bytes=%llu, down_bytes=%llu\n", 
        ...
        if (up_bytes > 0 || down_bytes > 0) {
            ...
            LOG_DEBUG("hos_api_get_daily_top_users: added user %s (mac=%s, total_bytes=%llu)\n", 
    ...
    LOG_DEBUG("hos_api_get_daily_top_users: total user_count = %d\n", user_count);
    
    
    if (user_count > 0) {
        LOG_DEBUG("hos_api_get_daily_top_users: sorting %d users\n", user_count);
    ...
    LOG_DEBUG("hos_api_get_daily_top_users: return_count = %d\n", return_count);
    ...
    if (!users_array) {
        LOG_ERROR("hos_api_get_daily_top_users: failed to create users_array\n");
        json_object_put(data_obj);
        return hos_gen_api_response_data(API_CODE_ERROR, NULL);
    }
    ...
    for (i = 0; i < return_count; i++) {
        struct json_object *user_obj = json_object_new_object();
        if (!user_obj) {
            LOG_ERROR("hos_api_get_daily_top_users: failed to create user_obj[%d]\n", i);
            continue;
        }
        ...
        LOG_DEBUG("hos_api_get_daily_top_users: added user[%d]: mac=%s, total_bytes=%llu\n", 
                 i, user_traffic_list[i].mac, user_traffic_list[i].total_bytes);
    }
    
    json_object_object_add(data_obj, "users", users_array);
    
    LOG_DEBUG("hos_api_get_daily_top_users: success, returning %d users\n", return_count);
    return hos_gen_api_response_data(API_CODE_SUCCESS, data_obj);
}
...
struct json_object *hos_api_get_active_users(struct json_object *req_obj) {
	int i;
	int hour;
    struct json_object *data_obj = json_object_new_object();
    if (!data_obj) {
        LOG_ERROR("hos_api_get_active_users: failed to create data_obj\n");
        return hos_gen_api_response_data(API_CODE_ERROR, NULL);
    }
    ...
    LOG_DEBUG("hos_api_get_active_users: start, count=%d\n", count);
    ...
    list_for_each_entry(node, &client_list, client) {
        if (user_count >= 1024) {
            LOG_ERROR("hos_api_get_active_users: user_count reached max limit 1024\n");
            break;
        }
        ...
        LOG_DEBUG("hos_api_get_active_users: added user: mac=%s, total_rate=%u\n", 
    ...
    if (user_count > 0) {
        LOG_DEBUG("hos_api_get_active_users: sorting %d users\n", user_count);
    ...
    LOG_DEBUG("hos_api_get_active_users: return_count = %d\n", return_count);
    ...
    struct json_object *users_array = json_object_new_array();
    if (!users_array) {
        LOG_ERROR("hos_api_get_active_users: failed to create users_array\n");
        json_object_put(data_obj);
        return hos_gen_api_response_data(API_CODE_ERROR, NULL);
    }
    ...
    for (i = 0; i < return_count; i++) {
        struct json_object *user_obj = json_object_new_object();
        if (!user_obj) {
            LOG_ERROR("hos_api_get_active_users: failed to create user_obj[%d]\n", i);
            continue;
        }
        ...
        LOG_DEBUG("hos_api_get_active_users: added user[%d]: mac=%s, total_rate=%u\n", 
    ...
    LOG_DEBUG("hos_api_get_active_users: success, returning %d users\n", return_count);
    return hos_gen_api_response_data(API_CODE_SUCCESS, data_obj);
}


struct json_object *hos_api_get_user_basic_info(struct json_object *req_obj) {
    struct json_object *data_obj = json_object_new_object();
    int hour;
    if (!req_obj) {
        return hos_gen_api_response_data(API_CODE_ERROR, NULL);
...
struct json_object *hos_api_get_online_offline_records(struct json_object *req_obj) {
    struct json_object *data_obj = json_object_new_object();
    ...
    if (!json_object_object_get_ex(req_obj, "mac", &mac_obj) || !mac_obj) {
        LOG_ERROR("hos_api_get_online_offline_records: missing mac parameter\n");
        json_object_put(data_obj);
        return hos_gen_api_response_data(API_CODE_ERROR, NULL);
    }
    ...
    const char *mac = json_object_get_string(mac_obj);
    if (!mac || strlen(mac) == 0) {
        LOG_ERROR("hos_api_get_online_offline_records: invalid mac parameter\n");
        json_object_put(data_obj);
        return hos_gen_api_response_data(API_CODE_ERROR, NULL);
    }
    ...
    client_node_t *client = find_client_node(mac);
    if (!client) {
        LOG_ERROR("hos_api_get_online_offline_records: client not found for mac=%s\n", mac);
        json_object_put(data_obj);
        return hos_gen_api_response_data(API_CODE_ERROR, NULL);
    }
    ...
    if (!records_array) {
        LOG_ERROR("hos_api_get_online_offline_records: failed to create records_array\n");
        json_object_put(data_obj);
        return hos_gen_api_response_data(API_CODE_ERROR, NULL);
    }
    ...
    list_for_each_entry(record, &client->online_offline_records, record) {
        ...
        if (!record_obj) {
            LOG_ERROR("hos_api_get_online_offline_records: failed to create record_obj\n");
            continue;
        }
    ...
    LOG_DEBUG("hos_api_get_online_offline_records: success, returning %d records for mac=%s\n", record_count, mac);
    return hos_gen_api_response_data(API_CODE_SUCCESS, data_obj);
}

struct json_object *hos_api_get_user_records(struct json_object *req_obj) {
    struct json_object *data_obj = json_object_new_object();
...
struct json_object *hos_api_get_record_base(struct json_object *req_obj) {
    struct json_object *data_obj = json_object_new_object();
    ...
    int enable = hos_uci_get_int_value(ctx, "hos.record.enable");
    int record_time = hos_uci_get_int_value(ctx, "hos.record.record_time");
    int app_valid_time = hos_uci_get_int_value(ctx, "hos.record.app_valid_time");
    
    char history_data_size[64] = {0};
    char history_data_path[256] = {0};
    hos_uci_get_value(ctx, "hos.record.history_data_size", history_data_size, sizeof(history_data_size));
    hos_uci_get_value(ctx, "hos.record.history_data_path", history_data_path, sizeof(history_data_path));
...
struct json_object *hos_api_set_record_base(struct json_object *req_obj) {
    struct uci_context *ctx = uci_alloc_context();
    if (!ctx) {
        return hos_gen_api_response_data(API_CODE_ERROR, NULL);
    }
    ...
    int num = hos_uci_get_list_num(ctx, "hos", "record");
    if (num <= 0) {
        hos_uci_add_section(ctx, "hos", "record");
    }

    char buf[16];
    snprintf(buf, sizeof(buf), "%d", enable);
    hos_uci_set_value(ctx, "hos.record.enable", buf);
    snprintf(buf, sizeof(buf), "%d", record_time);
    hos_uci_set_value(ctx, "hos.record.record_time", buf);
    snprintf(buf, sizeof(buf), "%d", app_valid_time);
    hos_uci_set_value(ctx, "hos.record.app_valid_time", buf);
    
    if (history_data_size && strlen(history_data_size) > 0) {
        hos_uci_set_value(ctx, "hos.record.history_data_size", (char *)history_data_size);
    } else {
        hos_uci_delete(ctx, "hos.record.history_data_size");
    }
    
    char old_path[256] = {0};
    hos_uci_get_value(ctx, "hos.record.history_data_path", old_path, sizeof(old_path));
    
    hos_uci_set_value(ctx, "hos.record.history_data_path", (char *)history_data_path);

    hos_uci_commit(ctx, "hos");
    ...
    update_hos_proc_u32_value("record_enable", enable);
...
struct json_object *hos_api_record_action(struct json_object *req_obj) {
    if (!req_obj) {
        return hos_gen_api_response_data(API_CODE_ERROR, NULL);
    }
...
struct json_object *hos_api_get_device_list(struct json_object *req_obj) {
    LOG_DEBUG("hos_api_get_device_list: called\n");
    ...
    LOG_DEBUG("hos_api_get_device_list: returning %d devices\n", json_object_array_length(device_array));
    return hos_gen_api_response_data(API_CODE_SUCCESS, data_obj);
}


struct json_object *hos_api_get_dashboard_param(struct json_object *req_obj) {
    LOG_DEBUG("hos_api_get_dashboard_param: called\n");
    ...
    LOG_DEBUG("hos_api_get_dashboard_param: monitor_device=%s\n", monitor_device);
    return hos_gen_api_response_data(API_CODE_SUCCESS, data_obj);
}

struct json_object *hos_api_set_dashboard_param(struct json_object *req_obj) {
    ...
    if (monitor_device && strlen(monitor_device) > 0) {
        hos_uci_set_value(uci_ctx, "hos.dashboard.monitor_device", (char *)monitor_device);
        hos_uci_commit(uci_ctx, "hos");
    ...
    LOG_DEBUG("hos_api_set_dashboard_param: set monitor_device=%s\n", monitor_device ? monitor_device : "(empty)");
    return hos_gen_api_response_data(API_CODE_SUCCESS, NULL);
}

typedef struct json_object * (*hos_api_handler)(struct json_object *data_obj);
...
typedef struct hos_api_node{
    char *api_name;
    hos_api_handler handler;
    int forward;      
    hos_api_method_t method;
}hos_api_node_t;

static void hos_forward_to_agent(struct json_object *req_obj) {
	char *cmd_buf = NULL;
	if (!req_obj)
		return;
	...
	if (!cmd_buf)
		return;
    snprintf(cmd_buf, buf_len, "ubus -t 2 call hos_agent forward '%s'", req_str);
...
static hos_api_node_t hos_api_node_list[] = {
    {"get_dashboard_common", hos_api_get_dashboard_common, 0, FWX_API_METHOD_GET},
    {"get_history_session", hos_api_get_history_session, 0, FWX_API_METHOD_GET},
    {"get_hourly_top_apps", hos_api_get_hourly_top_apps, 0, FWX_API_METHOD_GET},
    {"get_daily_top_apps", hos_api_get_daily_top_apps, 0, FWX_API_METHOD_GET},
    {"delete_record_files", hos_api_delete_record_files, 0, FWX_API_METHOD_POST},
    {"get_global_app_type_stats", hos_api_get_global_app_type_stats, 0, FWX_API_METHOD_GET},
    {"get_global_traffic_stats", hos_api_get_global_traffic_stats, 0, FWX_API_METHOD_GET},
    {"get_daily_top_users", hos_api_get_daily_top_users, 0, FWX_API_METHOD_GET},
    {"get_active_users", hos_api_get_active_users, 0, FWX_API_METHOD_GET},
    {"get_filter_rules", hos_api_get_filter_rules, 0, FWX_API_METHOD_GET},
    {"add_filter_rule", hos_api_add_filter_rule, 1, FWX_API_METHOD_POST},
    {"update_filter_rule", hos_api_update_filter_rule, 1, FWX_API_METHOD_POST},
    {"delete_filter_rule", hos_api_delete_filter_rule, 1, FWX_API_METHOD_POST},
    {"get_user_basic_info", hos_api_get_user_basic_info, 0, FWX_API_METHOD_GET},
    {"get_online_offline_records", hos_api_get_online_offline_records, 0, FWX_API_METHOD_GET},
    {"get_user_records", hos_api_get_user_records, 0, FWX_API_METHOD_GET},
    {"get_appfilter_whitelist", hos_api_get_appfilter_whitelist, 0, FWX_API_METHOD_GET},
    {"add_appfilter_whitelist", hos_api_add_appfilter_whitelist, 1, FWX_API_METHOD_POST},
    {"del_appfilter_whitelist", hos_api_del_appfilter_whitelist, 1, FWX_API_METHOD_POST},
    {"get_app_filter_adv", hos_api_get_app_filter_adv, 0, FWX_API_METHOD_GET},
    {"set_app_filter_adv", hos_api_set_app_filter_adv, 1, FWX_API_METHOD_POST},
    {"get_system_info", hos_api_get_system_info, 0, FWX_API_METHOD_GET},
    {"set_system_info", hos_api_set_system_info, 1, FWX_API_METHOD_POST},
    {"get_mac_filter_rules", hos_api_get_mac_filter_rules, 0, FWX_API_METHOD_GET},
    {"add_mac_filter_rule", hos_api_add_mac_filter_rule, 1, FWX_API_METHOD_POST},
    {"update_mac_filter_rule", hos_api_update_mac_filter_rule, 1, FWX_API_METHOD_POST},
    {"delete_mac_filter_rule", hos_api_delete_mac_filter_rule, 1, FWX_API_METHOD_POST},
    {"get_mac_filter_whitelist", hos_api_get_mac_filter_whitelist, 0, FWX_API_METHOD_GET},
    {"add_mac_filter_whitelist", hos_api_add_mac_filter_whitelist, 1, FWX_API_METHOD_POST},
    {"del_mac_filter_whitelist", hos_api_del_mac_filter_whitelist, 1, FWX_API_METHOD_POST},
    {"get_mac_filter_adv", hos_api_get_mac_filter_adv, 0, FWX_API_METHOD_GET},
    {"set_mac_filter_adv", hos_api_set_mac_filter_adv, 1, FWX_API_METHOD_POST},
    {"get_record_base", hos_api_get_record_base, 0, FWX_API_METHOD_GET},
    {"set_record_base", hos_api_set_record_base, 1, FWX_API_METHOD_POST},
    {"record_action", hos_api_record_action, 0, FWX_API_METHOD_POST},
    {"get_lan_list", hos_api_get_lan_list, 0, FWX_API_METHOD_GET},
    {"add_lan", hos_api_add_lan, 1, FWX_API_METHOD_POST},
    {"mod_lan", hos_api_mod_lan, 1, FWX_API_METHOD_POST},
    {"del_lan", hos_api_del_lan, 1, FWX_API_METHOD_POST},
    {"get_wan_list", hos_api_get_wan_list, 0, FWX_API_METHOD_GET},
    {"add_wan", hos_api_add_wan, 1, FWX_API_METHOD_POST},
    {"mod_wan", hos_api_mod_wan, 1, FWX_API_METHOD_POST},
    {"del_wan", hos_api_del_wan, 1, FWX_API_METHOD_POST},
    {"get_lan_info", hos_api_get_lan_info, 0, FWX_API_METHOD_GET},
    {"set_lan_info", hos_api_set_lan_info, 1, FWX_API_METHOD_POST},
    {"get_wan_info", hos_api_get_wan_info, 0, FWX_API_METHOD_GET},
    {"set_wan_info", hos_api_set_wan_info, 1, FWX_API_METHOD_POST},
    {"get_work_mode", hos_api_get_work_mode, 0, FWX_API_METHOD_GET},
    {"set_work_mode", hos_api_set_work_mode, 1, FWX_API_METHOD_POST},
    {"set_nickname", hos_api_set_nickname, 1, FWX_API_METHOD_POST},
    {"dev_visit_list", hos_api_dev_visit_list, 0, FWX_API_METHOD_GET},
    {"dev_visit_time", hos_api_dev_visit_time, 0, FWX_API_METHOD_GET},
    {"app_class_visit_time", hos_api_app_class_visit_time, 0, FWX_API_METHOD_GET},
    {"dev_list", hos_api_dev_list, 0, FWX_API_METHOD_GET},
    {"class_list", hos_api_class_list, 0, FWX_API_METHOD_GET},
    {"get_all_users", hos_api_get_all_users, 0, FWX_API_METHOD_GET},
    {"get_oaf_status", hos_api_get_oaf_status, 0, FWX_API_METHOD_GET},
    {"visit_list", hos_api_visit_list, 0, FWX_API_METHOD_GET},
    {"get_device_list", hos_api_get_device_list, 0, FWX_API_METHOD_GET},
    {"get_dashboard_param", hos_api_get_dashboard_param, 0, FWX_API_METHOD_GET},
    {"set_dashboard_param", hos_api_set_dashboard_param, 1, FWX_API_METHOD_POST},
...
int ubus_handle_common(struct ubus_context *ctx, struct ubus_object *obj, struct ubus_request_data *req, 
    ...
    if (!req_obj) {
        ...
        ubus_response_json(ctx, req, hos_gen_api_response_data(API_CODE_ERROR, NULL));
        free(msg_obj_str);
        return 0;
    }
    ...
    if (api_obj) {
        ...
        struct json_object *error_response = hos_gen_api_response_data(API_CODE_ERROR, NULL);
    ...
    for (i = 0; i < sizeof(hos_api_node_list) / sizeof(hos_api_node_t); i++) {

        if (hos_api_node_list[i].api_name == NULL) {
            break;
        }
        if (strcmp(hos_api_node_list[i].api_name, api_name) == 0) {
            api_node = &hos_api_node_list[i];
        ...
        if (response_obj) {
            ...
            if (api_node->forward) {
                ...
                if (code_obj && json_object_get_int(code_obj) == API_CODE_SUCCESS) {
                    hos_forward_to_agent(req_obj);
                }
            }
        } else {
            LOG_ERROR("Handler returned NULL for API: %s\n", api_name);
            response_obj = hos_gen_api_response_data(API_CODE_ERROR, NULL);
            ubus_response_json(ctx, req, response_obj);
        }
    }
    else {
...
static struct ubus_method hos_object_methods[] = {
    UBUS_METHOD("common", ubus_handle_common, def_policy),
    UBUS_METHOD("debug", handle_debug, def_policy),
};


static struct ubus_object_type hos_object_type =
    UBUS_OBJECT_TYPE("hos", hos_object_methods);


static struct ubus_object hos_object = {
    .name = "hos",
    .type = &hos_object_type,
    .methods = hos_object_methods,
    .n_methods = ARRAY_SIZE(hos_object_methods),
};

static void hos_add_object(struct ubus_object *obj)
{
...
int hos_ubus_init(void)
{
    LOG_INFO("hos ubus init...\n");
