...
void hos_netlink_handler(struct uloop_fd *u, unsigned int ev)
{
    int ret;
    int i;
    ...
    struct hos_nl_msg_hdr *af_hdr = (struct hos_nl_msg_hdr *)kmsg;
    ...
    char *kdata = kmsg + sizeof(struct hos_nl_msg_hdr);
    ...
    if (ipv6_obj) {
        ...
        if (ipv6_str && strlen(ipv6_str) > 0) {
            ...
            LOG_DEBUG("hos_netlink: received ipv6=%s for %s\n", node->ipv6, mac);
        } else {
            node->ipv6[0] = '\0';
        }
    } else {
    ...
    if (active_obj) {
        ...
        LOG_DEBUG("hos_netlink: received active=%d for %s\n", node->active, mac);
    }
    ...
    LOG_DEBUG("hos_netlink: received flow data for %s: up_flow=%llu KB (%llu bytes), down_flow=%llu KB (%llu bytes)\n",
    ...
    if (hour >= 0 && hour < HOURS_PER_DAY) {
        ...
        if (node->online == 1) {
            if (today_stat) {
                ...
                LOG_DEBUG("hos_netlink: updated hourly_online_time[%d] for %s: +%d seconds\n",
        ...
        if (node->active == 1) {
            if (today_stat) {
                ...
                LOG_DEBUG("hos_netlink: updated hourly_active_time[%d] for %s: +%d seconds\n",
                         hour, mac, REPORT_INTERVAL_SECS);
            }
        }
    ...
    if (hour >= 0 && hour < HOURS_PER_DAY && (total_up_bytes > 0 || total_down_bytes > 0)) {
        if (today_stat) {
            ...
            LOG_DEBUG("hos_netlink: updated hourly_traffic[%d] for %s: up_bytes=%llu, down_bytes=%llu\n",
...
int hos_nl_send_msg_to_kernel(int fd, void *msg, int len)
{
    ...
    struct hos_nl_msg_hdr *hdr = (struct hos_nl_msg_hdr *)msg_buf;
    hdr->magic = 0xa0b0c0d0;
    hdr->len = len;
    char *p_data = msg_buf + sizeof(struct hos_nl_msg_hdr);
    memcpy(p_data, msg, len);

    memcpy(NLMSG_DATA(nlh), msg_buf, len + sizeof(struct hos_nl_msg_hdr));
