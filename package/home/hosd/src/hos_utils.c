...
int hos_send_msg_to_kernel(char *buf){
    ...
    if (access("/dev/hos", F_OK) != 0) {
        return 0; // Device doesn't exist, silently skip
    }
    
    FILE *fp = fopen("/dev/hos", "w");
    if (fp) {
        fprintf(fp, "%s", buf);
        fclose(fp);
    }
...
int hos_parse_time_str(const char *time_str, hos_time_period_t *periods, int max_periods) {
    ...
    while (time_period && period_count < max_periods) {
        hos_time_period_t *period = &periods[period_count];
        memset(period, 0, sizeof(hos_time_period_t));
...
void update_hos_proc_value(char *key, char *value){
    ...
    sprintf(file_path, "/proc/sys/hos/%s", key);

    af_read_file_value(file_path, old_value, sizeof(old_value));    
    if (strcmp(old_value, value) != 0){
        sprintf(cmd_buf, "echo %s >/proc/sys/hos/%s", value, key);
        system(cmd_buf);
    }
}
