...
static int replace_first_list_item(struct uci_context *ctx, const char *uci_key, const char *first_item)
{
    ...
    if (hos_uci_get_list_value(ctx, (char *)uci_key, list_buf, sizeof(list_buf), " ") == 0 && list_buf[0] != '\0') {
    ...
    hos_uci_delete(ctx, (char *)uci_key);
    hos_uci_add_list(ctx, (char *)uci_key, (char *)first_item);

    for (i = 1; i < item_count; i++) {
        if (items[i] && items[i][0] != '\0') {
            hos_uci_add_list(ctx, (char *)uci_key, items[i]);
        }
    }
...
static int ensure_hos_network_section(struct uci_context *ctx)
{
    struct uci_ptr ptr;
    if (uci_lookup_ptr(ctx, &ptr, "hos.network", true) == UCI_OK) {
        return 0;
    }
    struct uci_package *pkg = NULL;
    if (uci_load(ctx, "hos", &pkg) != UCI_OK) {
        LOG_ERROR("ensure_hos_network_section: load hos failed\n");
        return -1;
    }
    char path[64];
    snprintf(path, sizeof(path), "hos.network=network");
    if (uci_lookup_ptr(ctx, &ptr, path, true) != UCI_OK) {
        LOG_ERROR("ensure_hos_network_section: lookup ptr failed\n");
        uci_unload(ctx, pkg);
        return -1;
    }
    if (uci_set(ctx, &ptr) != UCI_OK) {
        LOG_ERROR("ensure_hos_network_section: set failed\n");
        if (ptr.p) uci_unload(ctx, ptr.p);
        return -1;
    }
    if (uci_save(ctx, ptr.p) != UCI_OK) {
        LOG_ERROR("ensure_hos_network_section: save failed\n");
        if (ptr.p) uci_unload(ctx, ptr.p);
        return -1;
    }
    if (ptr.p) uci_unload(ctx, ptr.p);
...
struct json_object *hos_api_get_lan_list(struct json_object *req_obj) {
    LOG_DEBUG("hos_api_get_lan_list: called\n");
    struct json_object *result = get_interface_list_by_type("lan");
    LOG_DEBUG("hos_api_get_lan_list: returning result\n");
    return result;
}


struct json_object *hos_api_get_wan_list(struct json_object *req_obj) {
    LOG_DEBUG("hos_api_get_wan_list: called\n");
    struct json_object *result = get_interface_list_by_type("wan");
    LOG_DEBUG("hos_api_get_wan_list: returning result\n");
    return result;
}
...
static struct json_object *add_or_mod_interface(struct json_object *req_obj, const char *iftype, int is_add) {
    ...
    hos_uci_set_value(ctx, uci_path, (char *)device);
    

    snprintf(uci_path, sizeof(uci_path), "network.%s.proto", name);
    hos_uci_set_value(ctx, uci_path, (char *)proto);
    ...
    if (strcmp(proto, "static") == 0) {
        ...
        if (ipaddr_obj) {
            if (strcmp(iftype, "lan") == 0) {
                ...
                snprintf(uci_path, sizeof(uci_path), "network.%s.ipaddr", name);
                hos_uci_set_value(ctx, uci_path, (char *)json_object_get_string(ipaddr_obj));
            }
        }
        
        if (strcmp(iftype, "lan") == 0) {
            snprintf(uci_path, sizeof(uci_path), "network.%s.netmask", name);
            hos_uci_delete(ctx, uci_path);
        } else if (netmask_obj) {
            snprintf(uci_path, sizeof(uci_path), "network.%s.netmask", name);
            hos_uci_set_value(ctx, uci_path, (char *)json_object_get_string(netmask_obj));
        }
        
        if (gateway_obj) {
            snprintf(uci_path, sizeof(uci_path), "network.%s.gateway", name);
            hos_uci_set_value(ctx, uci_path, (char *)json_object_get_string(gateway_obj));
        }
        ...
        if (username_obj) {
            ...
            hos_uci_set_value(ctx, uci_path, (char *)json_object_get_string(username_obj));
        }
        
        if (password_obj) {
            snprintf(uci_path, sizeof(uci_path), "network.%s.password", name);
            hos_uci_set_value(ctx, uci_path, (char *)json_object_get_string(password_obj));
        }
    }
    
    hos_uci_commit(ctx, "network");
    uci_free_context(ctx);
...
struct json_object *hos_api_add_lan(struct json_object *req_obj) {
    LOG_DEBUG("hos_api_add_lan called\n");
    return add_or_mod_interface(req_obj, "lan", 1);
}


struct json_object *hos_api_mod_lan(struct json_object *req_obj) {
    LOG_DEBUG("hos_api_mod_lan called\n");
    return add_or_mod_interface(req_obj, "lan", 0);
}


struct json_object *hos_api_add_wan(struct json_object *req_obj) {
    LOG_DEBUG("hos_api_add_wan called\n");
    return add_or_mod_interface(req_obj, "wan", 1);
}


struct json_object *hos_api_mod_wan(struct json_object *req_obj) {
    LOG_DEBUG("hos_api_mod_wan called\n");
...
struct json_object *hos_api_del_lan(struct json_object *req_obj) {
    LOG_DEBUG("hos_api_del_lan called\n");
    return del_interface(req_obj, "lan");
}


struct json_object *hos_api_del_wan(struct json_object *req_obj) {
    LOG_DEBUG("hos_api_del_wan called\n");
    return del_interface(req_obj, "wan");
}


struct json_object *hos_api_get_lan_info(struct json_object *req_obj) {
    LOG_DEBUG("hos_api_get_lan_info called\n");
    ...
    
    {
        ...
        if (hos_uci_get_list_value(ctx, "network.lan.ipaddr", ipaddr_list_buf, sizeof(ipaddr_list_buf), " ") == 0 &&
            ipaddr_list_buf[0] != '\0') {
            char *saveptr = NULL;
        ...
        if (!list_ok) {
            hos_uci_get_value(ctx, "network.lan.ipaddr", ipaddr_str, sizeof(ipaddr_str));
            hos_uci_get_value(ctx, "network.lan.netmask", netmask_str, sizeof(netmask_str));
        }
    }
    hos_uci_get_value(ctx, "network.lan.proto", proto_str, sizeof(proto_str));
    hos_uci_get_value(ctx, "network.lan.gateway", gateway_str, sizeof(gateway_str));
    

    char dns_list_buf[128] = {0};
    if (hos_uci_get_list_value(ctx, "network.lan.dns", dns_list_buf, sizeof(dns_list_buf), " ") == 0) {
        char *saveptr = NULL;
...
struct json_object *hos_api_set_lan_info(struct json_object *req_obj) {
    LOG_DEBUG("hos_api_set_lan_info called22\n");
    ...
    char test_buf[32] = {0};
    if (hos_uci_get_value(ctx, "network.lan.proto", test_buf, sizeof(test_buf)) != 0) {

        if (hos_uci_set_value(ctx, "network.lan", "interface") != 0) {
    ...
    if (proto_obj) {
        ...
        hos_uci_set_value(ctx, "network.lan.proto", (char *)proto);
    }
    ...
    if (ipaddr_obj) {
        ...
        if (ipaddr && ipaddr[0] != '\0') {
            ...
            snprintf(ipaddr_with_prefix, sizeof(ipaddr_with_prefix), "%s/%d", ipaddr, prefix);
            /* ͳһʹ�� list ipaddr��x.x.x.x/nn����ʽ��ֻ�滻��һ����������� */
            replace_first_list_item(ctx, "network.lan.ipaddr", ipaddr_with_prefix);
        }
    }
    
    hos_uci_delete(ctx, "network.lan.netmask");
    
    if (gateway_obj) {
        hos_uci_set_value(ctx, "network.lan.gateway", (char *)json_object_get_string(gateway_obj));
    }
    

    if (dns1_obj || dns2_obj) {
        hos_uci_delete(ctx, "network.lan.dns");
...
struct json_object *hos_api_get_wan_info(struct json_object *req_obj) {
    LOG_DEBUG("hos_api_get_wan_info called\n");
    ...
    hos_uci_get_value(ctx, "network.wan.ipaddr", ipaddr_str, sizeof(ipaddr_str));
    hos_uci_get_value(ctx, "network.wan.netmask", netmask_str, sizeof(netmask_str));
    hos_uci_get_value(ctx, "network.wan.proto", proto_str, sizeof(proto_str));
    hos_uci_get_value(ctx, "network.wan.gateway", gateway_str, sizeof(gateway_str));
    hos_uci_get_value(ctx, "network.wan.username", username_str, sizeof(username_str));
    hos_uci_get_value(ctx, "network.wan.password", password_str, sizeof(password_str));
    

    char dns_list_buf[128] = {0};
    if (hos_uci_get_list_value(ctx, "network.wan.dns", dns_list_buf, sizeof(dns_list_buf), " ") == 0) {
        char *saveptr = NULL;
        char *p = strtok_r(dns_list_buf, " ", &saveptr);
        if (p) {
            strncpy(dns1_str, p, sizeof(dns1_str) - 1);
...
struct json_object *hos_api_set_wan_info(struct json_object *req_obj) {
    LOG_DEBUG("hos_api_set_wan_info called\n");
    ...
    char test_buf[32] = {0};
    if (hos_uci_get_value(ctx, "network.wan.proto", test_buf, sizeof(test_buf)) != 0) {

        if (hos_uci_set_value(ctx, "network.wan", "interface") != 0) {
    ...
    if (proto_obj) {
        ...
        hos_uci_set_value(ctx, "network.wan.proto", (char *)proto);
    } else {

        char proto_buf[32] = {0};
        if (hos_uci_get_value(ctx, "network.wan.proto", proto_buf, sizeof(proto_buf)) == 0) {
            proto = proto_buf;
        }
    }
    ...
    if (strcmp(proto, "static") == 0) {
        ...
        if (ipaddr_obj) {
            hos_uci_set_value(ctx, "network.wan.ipaddr", (char *)json_object_get_string(ipaddr_obj));
        }
        
        if (netmask_obj) {
            hos_uci_set_value(ctx, "network.wan.netmask", (char *)json_object_get_string(netmask_obj));
        }
        
        if (gateway_obj) {
            hos_uci_set_value(ctx, "network.wan.gateway", (char *)json_object_get_string(gateway_obj));
        }

        if (dns1_obj || dns2_obj) {
            hos_uci_delete(ctx, "network.wan.dns");
        ...
        if (username_obj) {
            ...
            if (username && strlen(username) > 0) {
                ...
                hos_uci_set_value(ctx, "network.wan.username", (char *)username);
            }
        }
        ...
        if (password_obj) {
            ...
            if (password && strlen(password) > 0) {
                ...
                hos_uci_set_value(ctx, "network.wan.password", (char *)password);
            }
        }
...
struct json_object *hos_api_get_work_mode(struct json_object *req_obj)
{
    LOG_DEBUG("hos_api_get_work_mode called\n");
    struct uci_context *ctx = uci_alloc_context();
    if (!ctx) {
        LOG_ERROR("hos_api_get_work_mode: alloc ctx failed\n");
        return hos_gen_api_response_data(API_CODE_ERROR, NULL);
    }
    int work_mode = hos_uci_get_int_value(ctx, "hos.network.work_mode");
...
struct json_object *hos_api_set_work_mode(struct json_object *req_obj)
{
    LOG_DEBUG("hos_api_set_work_mode called\n");
    if (!req_obj) {
        return hos_gen_api_response_data(API_CODE_ERROR, NULL);
    }
    struct json_object *wm_obj = json_object_object_get(req_obj, "work_mode");
    if (!wm_obj) {
        LOG_ERROR("hos_api_set_work_mode: missing work_mode\n");
        return hos_gen_api_response_data(API_CODE_ERROR, NULL);
    }
    int work_mode = json_object_get_int(wm_obj);
    if (work_mode != 0 && work_mode != 1) {
        LOG_ERROR("hos_api_set_work_mode: invalid work_mode %d\n", work_mode);
        return hos_gen_api_response_data(API_CODE_ERROR, NULL);
    }

    struct uci_context *ctx = uci_alloc_context();
    if (!ctx) {
        LOG_ERROR("hos_api_set_work_mode: alloc ctx failed\n");
        return hos_gen_api_response_data(API_CODE_ERROR, NULL);
    }

    hos_uci_set_int_value(ctx, "hos.network.work_mode", work_mode);
    hos_uci_commit(ctx, "hos");
    uci_free_context(ctx);

	update_hos_proc_u32_value("work_mode", work_mode);

    LOG_DEBUG("hos_api_set_work_mode: work_mode=%d\n", work_mode);
    return hos_gen_api_response_data(API_CODE_SUCCESS, NULL);
}
...
static void fill_lan_dhcp_info(struct json_object *data_obj)
{
    ...
    if (hos_uci_get_value(ctx, "dhcp.lan.ignore", val_buf, sizeof(val_buf)) == 0) {
        if (strcmp(val_buf, "1") == 0) enable = 0;
    }
    if (hos_uci_get_value(ctx, "dhcp.lan.start", val_buf, sizeof(val_buf)) == 0) {
        start = atoi(val_buf);
    }
    if (hos_uci_get_value(ctx, "dhcp.lan.limit", val_buf, sizeof(val_buf)) == 0) {
        limit = atoi(val_buf);
    }
    if (hos_uci_get_value(ctx, "dhcp.lan.leasetime", lease_str, sizeof(lease_str)) == 0) {
...
static int ensure_dhcp_lan_section(struct uci_context *ctx)
{
    ...
    hos_uci_set_value(ctx, "dhcp.lan.interface", "lan");
    return 0;
}
