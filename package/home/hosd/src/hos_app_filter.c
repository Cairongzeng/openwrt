...
hos_run_time_status_t g_af_status;
...
static int find_rule_index_by_id(struct uci_context *uci_ctx, int id) {
    char id_str_uci[32];
	int i;
    int num = hos_uci_get_list_num(uci_ctx, "appfilter", "rule");
    for (i = 0; i < num; i++) {
        char buf[128];
...
struct json_object *hos_api_get_filter_rules(struct json_object *req_obj) {
    ...
    uci_foreach_element(&pkg->sections, e) {
        ...
        // Process app_id list��֧���ַ����θ�ʽ��1001-1005��
...
struct json_object *hos_api_add_filter_rule(struct json_object *req_obj) {
    ...
    hos_uci_add_section(uci_ctx, "appfilter", "rule");
    ...
    snprintf(buf, sizeof(buf), "appfilter.@rule[-1].filter_quic");
    {
        ...
        hos_uci_set_value(uci_ctx, buf, filter_quic_str);
    }
    ...
    for (i = 0; i < app_ids_len; i++) {
        ...
        if (normalize_app_id_token(raw, app_id_str, sizeof(app_id_str))) {
            ...
            hos_uci_add_list(uci_ctx, buf, app_id_str);
        }
    }
...
struct json_object *hos_api_get_appfilter_whitelist(struct json_object *req_obj){
    ...
    char mac_str[128] = {0};
    int num = hos_uci_get_list_num(uci_ctx, "appfilter_whitelist", "whitelist_mac");
    for (i = 0; i < num; i++) {
        hos_uci_get_array_value(uci_ctx, "appfilter_whitelist.@whitelist_mac[%d].mac", i, mac_str, sizeof(mac_str));
...
struct json_object *hos_api_del_appfilter_whitelist(struct json_object *req_obj){
	int i;
    LOG_DEBUG("hos_api_del_appfilter_whitelist\n");
    ...
    hos_uci_commit(uci_ctx, "appfilter_whitelist");
    uci_free_context(uci_ctx);
...
struct json_object *hos_api_add_appfilter_whitelist(struct json_object *req_obj){
	int i;
    LOG_DEBUG("hos_api_add_appfilter_whitelist\n");
    ...
    for (i = 0; i < len; i++) {
        ...
        hos_uci_add_section(uci_ctx, "appfilter_whitelist", "whitelist_mac");
        hos_uci_set_value(uci_ctx, "appfilter_whitelist.@whitelist_mac[-1].mac", json_object_get_string(mac_obj));
    }
    hos_uci_commit(uci_ctx, "appfilter_whitelist");
    uci_free_context(uci_ctx);
...
struct json_object *hos_api_get_app_filter_adv(struct json_object *req_obj) {
    ...
    int enable = hos_uci_get_int_value(uci_ctx, "hos.appfilter.enable");
...
struct json_object *hos_api_set_app_filter_adv(struct json_object *req_obj) {
    ...
    hos_uci_set_int_value(uci_ctx, "hos.appfilter.enable", enable_value);
