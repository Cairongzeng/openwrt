...
const char *get_client_data_base_dir(void) {
    if (!g_client_data_base_dir_initialized) {
        ...
        if (uci_ctx) {
            ...
            int ret = hos_uci_get_value(uci_ctx, "hos.record.history_data_path", history_data_path, sizeof(history_data_path));
...
void load_app_valid_time_config(void) {
    struct uci_context *uci_ctx = uci_alloc_context();
    if (uci_ctx) {
        int app_valid_time = hos_uci_get_int_value(uci_ctx, "hos.record.app_valid_time");
        if (app_valid_time > 0) {
...
static void cleanup_expired_files_by_days(void) {
    ...
    int record_time = hos_uci_get_int_value(uci_ctx, "hos.record.record_time");
    uci_free_context(uci_ctx);
...
void check_and_cleanup_history_data_by_size(void) {
    ...
    char history_data_path[256] = {0};
    hos_uci_get_value(uci_ctx, "hos.record.history_data_size", history_data_size, sizeof(history_data_size));
    hos_uci_get_value(uci_ctx, "hos.record.history_data_path", history_data_path, sizeof(history_data_path));
    uci_free_context(uci_ctx);
...
void update_client_nickname(void)
{
    ...
    for (i = 0; i < num; i++) {
        ...
        hos_uci_get_array_value(uci_ctx, "user_info.@user_info[%d].nickname", i, nickname_buf, sizeof(nickname_buf));
