...
refresh_system_info() {                                                                                  
    ...
    work_mode=$(uci get hos.network.work_mode 2>/dev/null)
    if [ -z "$work_mode" ]; then
        work_mode="0"
    fi
    if [ "$work_mode" = "0" ]; then
...
check_and_init_network() {
    ...
    init_flag=$(uci get hos.global.cli_init 2>/dev/null)
    if [ "$init_flag" = "1" ]; then
        return 0
    fi
    ...
    if [ -z "$wan_proto" ]; then
        ...
        uci commit network
        uci set hos.global.cli_init=1
        uci set hos.network.work_mode=1
        uci commit hos
        /etc/init.d/network reload
		/etc/init.d/hos restart >/dev/null 2>&1
    else
        uci set hos.global.cli_init=1
        uci commit hos
    fi
...
monitor_clients() {
    ...
    while true; do
        ...
        while true; do
            ...
            json_data=$(ubus call hos common "{\"api\":\"get_all_users\",\"data\":{\"flag\":3,\"page\":$page,\"page_size\":20}}" 2>/dev/null)
