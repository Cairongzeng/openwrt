...
local function write_to_dev_hos(json_str)
    local dev_file = "/dev/hos"
...
local function delete_appfilter_rule(rule_id)
    ...
    if write_to_dev_hos(json_str) then
...
local function set_macfilter_rule_mac_list(rule_id, mac_list)
    ...
    if not write_to_dev_hos(clear_json) then
...
local function apply_appfilter_enable(enable)
    ...
    local proc_file = "/proc/sys/hos/appfilter_enable"
...
local function apply_macfilter_enable(enable)
    ...
    local proc_file = "/proc/sys/hos/macfilter_enable"
    local file = io.open(proc_file, "w")
    if file then
        file:write(tostring(enable))
        file:close()
...
local function check_and_apply_appfilter_enable()
    local current_enable = get_uci_enable("hos", "appfilter", "enable")
...
local function check_and_apply_macfilter_enable()
    local current_enable = get_uci_enable("hos", "macfilter", "enable")
...
local function apply_record_enable(enable)
    ...
    local proc_file = "/proc/sys/hos/record_enable"
    local file = io.open(proc_file, "w")
    if file then
        file:write(tostring(enable))
        file:close()
...
local function check_and_apply_record_enable()
    local current_enable = get_uci_enable("hos", "record", "enable")
