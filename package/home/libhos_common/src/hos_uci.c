...
int hos_uci_get_array_value(struct uci_context *ctx, char *key_fmt, int index, char *output, int out_len)
{
    char key[128] = {0};
    sprintf(key, key_fmt, index);
    return hos_uci_get_value(ctx, key, output, out_len);
}
...
static struct uci_package *hos_uci_get_package(const char *config)
{
