...
#include "hos_client_fs.h"
#include "k_json.h"
#include "hos_conntrack.h"
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Powered by HOS");
MODULE_DESCRIPTION("hos module");
...
#define SET_APPID(ct, appid) ((ct)->hos_data.app_id = (appid))
#define GET_APPID(ct) ((ct)->hos_data.app_id)
...
int hos_match_feature(flow_info_t *flow)
{
...
int match_app_filter_rule(int appid, af_client_info_t *client)
{
	...
	if (hos_match_app_filter_whitelist(client->mac)){
		AF_LMT_DEBUG("match appfilter whitelist mac = " MAC_FMT "\n", MAC_ARRAY(client->mac));
		return AF_FALSE;
	}

	app_filter_rule_t *rule = hos_match_app_filter_rule(appid, client->mac);
	if (rule) {
...
int match_mac_filter_rule(af_client_info_t *client)
{
	...
	if (hos_match_mac_filter_whitelist(client->mac)){
		AF_LMT_DEBUG("match macfilter whitelist mac = " MAC_FMT "\n", MAC_ARRAY(client->mac));
		return AF_FALSE;
	}

	mac_filter_rule_t *rule = hos_match_mac_filter_rule(client->mac);
	if (rule) {
...
u_int32_t hos_hook_bypass_handle(struct sk_buff *skb, struct net_device *dev)
{
	flow_info_t flow;
	af_conn_t *conn;
	...
	if (!skb || !dev)
		return NF_ACCEPT;
	if (0 == hos_lan_ip || 0 == hos_lan_mask)
		return NF_ACCEPT;
	...
	{
		if (hos_lan_ip == flow.src || hos_lan_ip == flow.dst)
		{
			return NF_ACCEPT;
		}
		...
		if ((flow.src & hos_lan_mask) != (hos_lan_ip & hos_lan_mask))
		{
			return NF_ACCEPT;
		}
	}
...
u_int32_t hos_hook_gateway_handle(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (ct->hos_data.app_id != 0)
	{
		app_id = ct->hos_data.app_id;
		
		AF_LMT_DEBUG("ct appid = %d\n", app_id);
		u_int32_t orig_action = ct->hos_data.action;

		int ct_action = ct->hos_data.action;
		flow.ignore = (ct->hos_data.match_status & 0x1) ? 1 : 0;
		...
		if (app_id > 1000 && app_id < 32000)
		{
			if (check_app_action_changed(ct_action, app_id, client)){
				ct_action = !ct_action;
				ct->hos_data.action = ct_action;
				AF_LMT_DEBUG("update appid %d action to %s, action = %d-->%d\n",
					 app_id, ct_action ? "drop" : "accept", orig_action, ct->hos_data.action);
			}
	...
	if (ct->hos_data.match_status & 0x2) {	
		flow.client_hello = 1;
	}
	...
	if (hos_match_feature(&flow)){
		ct->hos_data.app_id = flow.app_id;
		if (flow.app_id < 1000){
			flow.ignore = 1;
		}
		else if (flow.feature && flow.feature->ignore){
			ct->hos_data.match_status |= 0x1;  
			flow.ignore = 1;
			AF_LMT_DEBUG("gateway set ignore bit, match_status = %u\n", ct->hos_data.match_status);
		}
		
		if (match_app_filter_rule(flow.app_id, client)) {
			flow.drop = 1;
			ct->hos_data.action = 1;  
			AF_LMT_INFO("##Drop App filter rule, appid = %d, mac = " MAC_FMT "\n", 
					flow.app_id, MAC_ARRAY(client->mac));
			if (skb->protocol == htons(ETH_P_IP) && g_tcp_rst){
			#if LINUX_VERSION_CODE > KERNEL_VERSION(5,10,197)
...
static u_int32_t hos_hook(void *priv,
								 struct sk_buff *skb,
								 const struct nf_hook_state *state)
{
#else
static u_int32_t hos_hook(unsigned int hook,
								 struct sk_buff *skb,
								 ...
								 int (*okfn)(struct sk_buff *))
{
	...
	if (AF_MODE_BYPASS == af_work_mode)
		return NF_ACCEPT;
	return hos_hook_gateway_handle(skb, skb->dev);
}

#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 4, 0)
static u_int32_t hos_by_pass_hook(void *priv,
										 struct sk_buff *skb,
										 const struct nf_hook_state *state)
{
#else
static u_int32_t hos_by_pass_hook(unsigned int hook,
										 struct sk_buff *skb,
										 const struct net_device *in,
										 const struct net_device *out,
										 int (*okfn)(struct sk_buff *))
{
	...
	if (AF_MODE_GATEWAY == af_work_mode)
		return NF_ACCEPT;
	return hos_hook_bypass_handle(skb, skb->dev);
}

#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 16, 0)
static struct nf_hook_ops hos_ops[] __read_mostly = {
	{
		.hook = hos_hook,
		.pf = NFPROTO_INET,
...
struct timer_list hos_timer;
int report_flag = 0;
#define FWX_TIMER_INTERVAL 1
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 15, 0)
static void hos_timer_func(struct timer_list *t)
#else
static void hos_timer_func(unsigned long ptr)
#endif
{
	static int count = 0;
	if (count % 60 == 0)
	...
	mod_timer(&hos_timer, jiffies + FWX_TIMER_INTERVAL * HZ);
}

void init_hos_timer(void)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 15, 0)
	timer_setup(&hos_timer, hos_timer_func, 0);
#else
	setup_timer(&hos_timer, hos_timer_func, FWX_TIMER_INTERVAL * HZ);
#endif
	mod_timer(&hos_timer, jiffies + FWX_TIMER_INTERVAL * HZ);
	AF_INFO("init hos timer...ok");
}

void fini_hos_timer(void)
{
	del_timer_sync(&hos_timer);
	AF_INFO("del hos timer...ok");
}

	static struct sock *hos_sock = NULL;
...
int af_send_msg_to_user(char *pbuf, uint16_t len)
{
	...
	ret = netlink_unicast(hos_sock, nl_skb, 999, MSG_DONTWAIT);
...
static void hos_user_msg_handle(char *data, int len)
{
...
static void hos_netlink_msg_rcv(struct sk_buff *skb)
{
	...
	if (skb->len >= nlmsg_total_size(0))
	{
		...
		if (udata)
			hos_user_msg_handle(udata, af_hdr->len);
	}
}

int netlink_hos_init(void)
{
	struct netlink_kernel_cfg nl_cfg = {0};
	nl_cfg.input = hos_netlink_msg_rcv;
	hos_sock = netlink_kernel_create(&init_net, FWX_NETLINK_ID, &nl_cfg);

	if (NULL == hos_sock)
	{
		AF_ERROR("init hos netlink failed, id=%d\n", FWX_NETLINK_ID);
		return -1;
	}
	AF_INFO("init hos netlink ok, id = %d\n", FWX_NETLINK_ID);
	return 0;
}
...
static int __init hos_init(void)
{
	int err;
	af_conn_init();
	netlink_hos_init();
	...
	err = nf_register_net_hooks(&init_net, hos_ops, ARRAY_SIZE(hos_ops));
#else
	err = nf_register_hooks(hos_ops, ARRAY_SIZE(hos_ops));
#endif
	if (err)
	{
		AF_ERROR("hos register filter hooks failed!\n");
	}
	init_hos_timer();
	printk("hos: Driver ver. %s - Powered by HOS\n", AF_VERSION);
	printk("hos: init ok\n");
	return 0;
}

static void hos_fini(void)
{
	AF_INFO("hos module exit\n");
	fini_hos_timer();
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 3, 0)
	nf_unregister_net_hooks(&init_net, hos_ops, ARRAY_SIZE(hos_ops));
#else
	nf_unregister_hooks(hos_ops, ARRAY_SIZE(hos_ops));
#endif
	...
	hos_unregister_dev();
	if (hos_sock)
		netlink_kernel_release(hos_sock);
	af_conn_exit();
	return;
}
...
module_init(hos_init);
module_exit(hos_fini);
