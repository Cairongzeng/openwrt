...
int hos_mac_filter_init(void) {
	...
	g_mac_rule_count = 0;
	hos_mac_config_init(&g_mac_filter_whitelist);
...
void hos_mac_filter_exit(void) {
	...
	g_mac_rule_count = 0;
	hos_flush_mac_list(&g_mac_filter_whitelist);
	mac_filter_write_unlock();
}
...
int hos_add_mac_filter_rule(int rule_id) {
	...
	if (hos_find_mac_filter_rule(rule_id)) {
		return -1;
	}
...
int hos_del_mac_filter_rule(int rule_id) {
	...
	mac_filter_write_lock();
	rule = hos_find_mac_filter_rule(rule_id);
	if (rule) {
		hos_flush_mac_list(&rule->mac_list);
		list_del(&rule->list);
		kfree(rule);
...
int hos_add_mac_to_rule(int rule_id, const unsigned char *mac) {
	...
	rule = hos_find_mac_filter_rule(rule_id);
	if (rule) {
		hos_add_mac_node(&rule->mac_list, mac);
		mac_filter_write_unlock();
		return 0;
	}
	mac_filter_write_unlock();
...
int hos_del_mac_from_rule(int rule_id, const unsigned char *mac) {
	...
	rule = hos_find_mac_filter_rule(rule_id);
	if (rule) {
		node = hos_find_mac_node(&rule->mac_list, mac);
		if (node) {
...
int hos_api_add_mac_filter_rule(cJSON *data_obj) {
	...
	if (hos_add_mac_filter_rule(rule_id_obj->valueint) < 0) {
		return -1;
	}
...
int hos_api_mod_mac_filter_rule(cJSON *data_obj) {
	...
	rule = hos_find_mac_filter_rule(rule_id_obj->valueint);
	if (!rule) {
...
int hos_api_del_mac_filter_rule(cJSON *data_obj) {
	...
	if (mac_obj) {
		...
		u8 mac_bin[ETH_ALEN] = {0};
		if (mac_str_to_bin(mac_obj->valuestring, mac_bin)) {
			return hos_del_mac_from_rule(rule_id_obj->valueint, mac_bin);
		}
	} else {

		return hos_del_mac_filter_rule(rule_id_obj->valueint);
	}
...
int hos_match_mac_filter_whitelist(const unsigned char *mac) {
	...
	mac_filter_read_lock();
	node = hos_find_mac_node(&g_mac_filter_whitelist, mac);
	ret = (node != NULL);
	mac_filter_read_unlock();
...
int hos_api_add_mac_filter_whitelist(cJSON *data_obj) {
	...
	for (i = 0; i < cJSON_GetArraySize(mac_array); i++) {
		...
		if (mac_obj && mac_str_to_bin(mac_obj->valuestring, mac_bin)) {
			hos_add_mac_node(&g_mac_filter_whitelist, mac_bin);
		}
	}
	mac_filter_write_unlock();
...
int hos_api_del_mac_filter_whitelist(cJSON *data_obj) {
	...
	mac_filter_write_lock();
	node = hos_find_mac_node(&g_mac_filter_whitelist, mac_bin);
	if (node) {
		hlist_del(&node->hlist);
		kfree(node);
		mac_filter_write_unlock();
