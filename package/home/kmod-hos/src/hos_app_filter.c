...
static int del_app_id_token_from_rule(int rule_id, cJSON *app_id_obj)
{
    ...
    if (app_id_obj->type == cJSON_Number) {
        return hos_del_app_id_from_rule(rule_id, app_id_obj->valueint);
    }
    ...
    if (sscanf(token, "%d-%d", &start_id, &end_id) == 2) {
        ...
        for (i = start_id; i <= end_id; i++) {
            hos_del_app_id_from_rule(rule_id, i);
        }
        return 0;
    }

    if (kstrtoint(token, 10, &single_id) == 0 && single_id > 0) {
        return hos_del_app_id_from_rule(rule_id, single_id);
    }
...
int hos_app_filter_init(void) {
    ...
    g_app_rule_count = 0;
    hos_mac_config_init(&g_app_filter_whitelist);
...
void hos_app_filter_exit(void) {
    ...
    list_for_each_entry_safe(rule, next, &app_filter_rule_list, list) {
        ...
        hos_flush_mac_list(&rule->mac_list);
        list_del(&rule->list);
        kfree(rule);
    }
    g_app_rule_count = 0;
    hos_flush_mac_list(&g_app_filter_whitelist);
    app_filter_write_unlock();
...
void hos_update_appfilter_jiffies(void){
...
int hos_add_app_filter_rule(int rule_id) {
    ...
    if (hos_find_app_filter_rule(rule_id)) {
    ...
    rule->enable = 1;
    rule->filter_quic = 0;
    hos_mac_config_init(&rule->mac_list);
...
int hos_del_app_filter_rule(int rule_id) {
    ...
    app_filter_write_lock();
    rule = hos_find_app_filter_rule(rule_id);
    if (rule) {
        flush_app_id_list(&rule->app_id_list);
        hos_flush_mac_list(&rule->mac_list);
        list_del(&rule->list);
...
app_filter_rule_t *hos_match_app_filter_rule(int app_id, const unsigned char *mac) {
    ...
    list_for_each_entry(rule, &app_filter_rule_list, list) {
        ...
        mac_node = hos_find_mac_node(&rule->mac_list, mac);
        if (!mac_node) {
...
int hos_api_add_app_filter_rule(cJSON *data_obj) {
	...
	hos_update_appfilter_jiffies();

    if (hos_add_app_filter_rule(rule_id_obj->valueint) < 0) {
        return -1;
    }
...
int hos_api_mod_app_filter_rule(cJSON *data_obj) {
    ...
    rule = hos_find_app_filter_rule(rule_id_obj->valueint);
    if (!rule) {
            ...
            if (mac_array) {
                ...
                for (i = 0; i < cJSON_GetArraySize(mac_array); i++) {
                    ...
                    if (mac_obj && mac_str_to_bin(mac_obj->valuestring, mac_bin)) {
                        hos_add_mac_node(&rule->mac_list, mac_bin);
                    }
                }
                app_filter_write_unlock();
            }
...
int hos_api_del_app_filter_rule(cJSON *data_obj) {
    ...
    if (app_id_obj) {
        ...
        return hos_del_app_filter_rule(rule_id_obj->valueint);
    }
}
...
int hos_match_app_filter_whitelist(const unsigned char *mac) {
    ...
    app_filter_read_lock();
    node = hos_find_mac_node(&g_app_filter_whitelist, mac);
    ret = (node != NULL);
    app_filter_read_unlock();
...
int hos_api_add_app_filter_whitelist(cJSON *data_obj) {
    ...
    for (i = 0; i < cJSON_GetArraySize(mac_array); i++) {
        cJSON *mac_obj = cJSON_GetArrayItem(mac_array, i);
        if (mac_obj && mac_str_to_bin(mac_obj->valuestring, mac_bin)) {
            hos_add_mac_node(&g_app_filter_whitelist, mac_bin);
        }
    }
    app_filter_write_unlock();
