...
m = Map("appfilter", translate("Feature Library"), translate("The feature library is used to describe app features, app filtering effect and number-dependent feature library"))
s = m:section(SimpleSection)
s.template = "hos_feature/feature"
s.anonymous = true
...
local function log(msg)
    ...
    if f then
        f:write(os.date("%Y-%m-%d %H:%M:%S") .. " [hos_feature_upload] " .. tostring(msg) .. "\n")
        f:close()
    end
end
...
http.setfilehandler(function(meta, chunk, eof)
    local feature_file = "/etc/hosd/feature.cfg"
    local f_format = "v3.0"
    local format = "v3.0"
    if not fd then
        if not meta then
    ...
    if eof and fd then
        ...
        if ret ~= nil then
            ...
            luci.sys.exec("killall -SIGUSR1 hosd")
